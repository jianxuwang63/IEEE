...
\end{equation*}
A linear classifier then projects $\mathbf{z}_i$ to a scalar logit, and a sigmoid activation yields the predicted bag-level probability $\hat{p}_i = \sigma\!\left(h_{\mathrm{c}}(\mathbf{z}_i)\right)$. This formulation provides a fully differentiable, permutation-invariant aggregation scheme that assigns interpretable importance weights to individual tiles.
