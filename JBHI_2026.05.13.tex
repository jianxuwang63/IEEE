...
\thanks{This research is supported by the Natural Science Foundation of Hunan Province 2024JJ6303, the Research Foundation of the Education Department of Hunan Province 23B0056, Hunan Provincial NSF Key Project 2025JJ30008, Hunan Furong Program High-Level Health Talent Support Project 20241226039 and High-Level Talent Support Program of Hunan Cancer Hospital 202507311010. (Corresponding author: Xuewei Cheng.)}
...
Effective clinical deployment of AI requires models that know when to abstain—often termed “intellectual humility” in high-stakes settings. Selective classification provides a mechanism for this by allowing a model to reject uncertain predictions and defer them to human experts, trading coverage for higher reliability on accepted cases \cite{hendrycks2016baseline, leibig2022combining, xia2024uncertainty}. Architecturally, SelectiveNet \cite{geifman2019selectivenet, geifman2017selective} jointly trains prediction and selection heads to optimize performance on the covered subset, an approach later extended to handle distribution shifts and hierarchical rejection schemes \cite{goren2024hierarchical}.


Conformal risk control (CRC) \cite{bates2021distribution, angelopoulos2022conformal} offers a complementary, post-hoc framework that provides distribution-free guarantees on user-defined risk metrics. CRC has been applied in medical imaging to enable reliable human-in-the-loop workflows \cite{dvijotham2023enhancing}. Inspired by CRC, we integrate risk control and FNR control into threshold calibration for selective classification. This allows SelectiveNet to learn representations of ambiguous inputs, while the NP umbrella algorithm provides statistical guarantees on clinical safety bounds.
...
\subsection{The Backbone: ViT-based MIL with Gated Attention Pooling} \label{sec:baseline_method}

We establish a multiple instance learning (MIL) framework in which a Vision Transformer (ViT)~\cite{dosovitskiy2020image} serves as the instance encoder and gated attention pooling~\cite{ilse2018attention} aggregates instance-level 
representations into a bag-level prediction. This backbone constitutes the foundation upon which our subsequent contributions are built.

\subsubsection{Bag Construction}

Each whole-slide image is decomposed into a bag of local tiles via a sliding window with overlap~\cite{campanella2019clinical,lu2021data}. Reflection 
padding ensures images below a minimum resolution are handled gracefully, and boundary-aligned tiling preserves peripheral tissue that would otherwise be truncated by a regular stride.

\subsubsection{Instance Feature Extraction}
Each tile $\mathbf{x}_k$ is encoded by a ViT pre-trained under the Phikon framework~\cite{filiot2023scaling}, with weights initialised via partial key-matching to accommodate architectural variants. The tile-level representation is obtained from the \(\texttt{[CLS]}\) token of the final 
transformer layer:
\begin{equation*}
    \mathbf{h}_k = f_{\text{ViT}}(\mathbf{x}_k;\,\theta_{\text{enc}}),
    \quad \mathbf{h}_k \in \mathbb{R}^{d_h},
\end{equation*}
where $d_h$ denotes the hidden dimension of the encoder.

\subsubsection{Gated Attention Aggregation and Bag-Level Prediction}
\label{sec:gated_attention}
Given the set of instance features $\mathcal{H}_i = \{\mathbf{h}_{i,j}\}_{j=1}^{n_i}$ for the $i$-th bag, unnormalized attention scores are computed via a gated mechanism that modulates a $\tanh$ branch with a sigmoid gate:
\begin{equation*}
    s_{i,j} = \mathbf{w}^{\top}
    \!\left(\tanh\!\left(V\mathbf{h}_{i,j}\right) \odot
    \sigma\!\left(U\mathbf{h}_{i,j}\right)\right),
\end{equation*}
where $V,\,U \in \mathbb{R}^{d_a \times d_h}$ and $\mathbf{w} \in \mathbb{R}^{d_a}$ are learnable parameters, and $d_a$ is the attention dimension. Scores are normalized via softmax to yield attention weights $\{a_{i,j}\}$, and the bag-level representation is formed as their weighted sum:
\begin{equation*}
    \mathbf{z}_i = \sum_{j=1}^{n_i} a_{i,j}\,\mathbf{h}_{i,j}.
\end{equation*}
A linear classifier then projects $\mathbf{z}_i$ to a scalar logit, and a sigmoid function yields the predicted bag-level probability $\hat{p}_i = \sigma\!\left(h_{\mathrm{c}}(\mathbf{z}_i)\right)$. This formulation provides a permutation-invariant aggregation mechanism that assigns interpretable importance weights to individual tiles, which is a desirable property for multiple instance learning.
...
We implement our framework in PyTorch and conduct all experiments on an NVIDIA RTX 5090 GPU (32 GB). The dataset comprises 7,133 locally collected pathology images with image-level labels indicating the presence ($y=1$) or absence ($y=0$) of MIP. Following common practice in computational pathology \cite{campanella2019clinical,lu2021data}, we partition each image into overlapping $512 \times 512$ tiles using a stride of 256, applying reflection padding at boundaries to ensure full coverage. Each tile is resized to $224 \times 224$ to preserve tissue-level context. To handle variable bag sizes in mini-batches, we employ zero-padding with an attention mask that excludes padded regions from gradient updates. All tiles are normalized using ImageNet statistics \cite{russakovsky2015imagenet}. During training, we apply strong data augmentation via Albumentations \cite{buslaev2020albumentations}, including random flips, moderate rotations, and color jitter.

We initialize the ViT-based MIL baseline with Phikon weights \cite{filiot2023scaling} and adopt gated attention pooling \cite{ilse2018attention}. The dataset is split into 80\% training and 20\% test using stratified sampling. All models are optimized with the same protocol: AdamW \cite{loshchilov2017decoupled} (learning rate $10^{-5}$, weight decay $0.05$) for 15 epochs, a cosine schedule with 10\% linear warmup \cite{loshchilov2016sgdr}, mixed precision, and gradient accumulation (batch size 8, accumulation steps 4). Regularization includes dropout of 0.2 and label smoothing of 0.1 \cite{szegedy2016rethinking}. To ensure stable and reproducible evaluation, we fix random seeds and enable deterministic cuDNN. Unless stated otherwise, results are reported using the final training checkpoint with a default decision threshold of $\tau = 0.5$.
...
To mitigate the high false negative rate (FNR) observed in the baseline, we adopt Cost-Sensitive Learning (CSL) \cite{he2009learning,elkan2001foundations}. For each positive-class weight $w_{1} \in \{5,10,15,20\}$, we train the same ViT-MIL architecture using a class-weighted cross-entropy loss, with the negative-class weight fixed at $w_{0}=1$. For evaluation, we maintain the same decision threshold $\tau = 0.5$. Figure~\ref{fig:main_diagram} illustrates the overall behavior across different cost scenarios.
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/Final_CSL_Figure.pdf} 
...
However, several issues merit further investigation. First, while the NP umbrella method can control the overall FNR at a specified level, which is an \textcolor{red}{algorithm and data independent}\textcolor{blue}{Two hyphens should be added here.} guarantee, it requires a separate set of positive validation samples. In clinical practice, positive samples are often the minority class, which can exacerbate training difficulty and compromise predictive accuracy. Second, although selective classification provides a theoretical lower bound on the risk–coverage tradeoff, controlling overall risk does not necessarily guarantee control of the FNR. \textcolor{red}{Although cost-sensitive learning (Case 2 in Section \ref{sec:phase3_method}) empirically reduces the FNR, its theoretical assurance remains an open question for future work.}\textcolor{blue}{There is an error in this statement, since Case 2 does not involve cost-sensitive learning (CSL).}
