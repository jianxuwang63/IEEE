...
class Config:
    ...
    # 🔥 关键切换：'baseline', 'clam_sb', 'dsmil', 'dtfd', 'transmil'
    arch_type = 'transmil'
...
# =========================================================
# 🌟 新增：TransMIL 专属核心模块 (金字塔位置编码器 PPEG)
# =========================================================
class PPEG(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # 将 1D 序列重塑为 2D 图像网格，以应用深度卷积提取空间特征
        cnn_feat = feat_token.transpose(1, 2).view(B, C, H, W)
        x = self.proj(cnn_feat) + cnn_feat + self.proj1(cnn_feat) + self.proj2(cnn_feat)

        # 展平回 1D 序列，并拼接回 CLS token
...
class FullSOTA_MIL(nn.Module):
    def __init__(self, arch_type='baseline', init_weights_path=None):
        ...
        if init_weights_path and os.path.exists(init_weights_path):
            ...
            print(f"⚠️ 未找到本地权重，使用随机初始化！")

        if arch_type == 'baseline':
            self.aggregator = nn.Sequential(nn.Linear(self.feature_dim, 256), nn.ReLU(), nn.Linear(256, 2))
        ...
        elif arch_type == 'transmil':
            # 🌟 TransMIL 专属网络结构
            self.trans_proj = nn.Linear(self.feature_dim, 512)
            self.cls_token = nn.Parameter(torch.randn(1, 1, 512))

            # Transformer 层 1
            encoder_layer1 = nn.TransformerEncoderLayer(d_model=512, nhead=8, dim_feedforward=512, dropout=0.1,
                                                        activation='relu', batch_first=True)
            self.trans_layer1 = nn.TransformerEncoder(encoder_layer1, num_layers=1)

            # PPEG 位置编码
            self.pos_layer = PPEG(dim=512)

            # Transformer 层 2
            encoder_layer2 = nn.TransformerEncoderLayer(d_model=512, nhead=8, dim_feedforward=512, dropout=0.1,
                                                        activation='relu', batch_first=True)
            self.trans_layer2 = nn.TransformerEncoder(encoder_layer2, num_layers=1)

            self.trans_norm = nn.LayerNorm(512)
            self.trans_classifier = nn.Linear(512, 2)

    def _smart_load(self, path):
        print(f"🔄 正在从本地注入 Phikon 权重: {path}...")
        try:
            ...
            print(f"✅ 成功加载了 {len(pretrained_dict)} 个 Phikon 权重！")
        except Exception as e:
            print(f"❌ 权重注入失败，报错详情: {e}")
    ...
    def forward(self, x, mask=None):
        ...
        if self.arch_type == 'clam_sb':
            ...
            if mask is not None: pos_scores = pos_scores.masked_fill(~mask, -1e4)
            ...
            if mask is not None: attn = attn.masked_fill(~mask.unsqueeze(1), -1e4)
        ...
        elif self.arch_type == 'transmil':
            # 🌟 TransMIL 核心前向传播逻辑
            h = self.trans_proj(feat)  # 降维到 [B, N, 512]

            # 计算 PPEG 需要的近似正方形的 H 和 W
            N = h.shape[1]
            H = int(np.ceil(np.sqrt(N)))
            W = H
            add_length = H * W - N

            # 如果不是完美的平方数，需要用前面的 Token 填补一下空缺
            if add_length > 0:
                h = torch.cat([h, h[:, :add_length, :]], dim=1)

            # 拼接 CLS Token
            cls_tokens = self.cls_token.expand(b, -1, -1)
            h = torch.cat((cls_tokens, h), dim=1)  # [B, N_pad+1, 512]

            # Transformer 交互 -> PPEG 注入空间信息 -> 再次 Transformer 交互
            h = self.trans_layer1(h)
            h = self.pos_layer(h, H, W)
            h = self.trans_layer2(h)
            h = self.trans_norm(h)

            # 取出头部的 CLS Token 用于分类判断
            logits = self.trans_classifier(h[:, 0])
...
def compute_sota_loss(logits, labels, aux_dict, arch_type, criterion):
    ...
    # TransMIL 和 Baseline 只需要计算纯净的 bag_loss
    return bag_loss
